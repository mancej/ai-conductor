...
describe('structural: smoke test entry point', () => {
  it('installs an owned temporary scope before loading and running the smoke command', async () => {
    ...
    expect({
      ...
      root: undefined,
      scope: expect.stringMatching(new RegExp(`^${selectedStorage}/ac-v-`)),
      tmpdir: expect.stringMatching(new RegExp(`^${selectedStorage}/ac-v-`)),
  ...
  it('selects the default checkout-local storage before loading the smoke command', async () => {
    ...
    try {
      ...
      expect(scope).toMatch(new RegExp(`^${join(conductorRoot, '.vitest-tmp', 'ac-v-')}`));
    } finally {
