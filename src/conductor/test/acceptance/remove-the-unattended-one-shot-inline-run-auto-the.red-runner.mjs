...
const target =
  'test/acceptance/remove-the-unattended-one-shot-inline-run-auto-the.acceptance.test.ts';
const reportPath = join(process.cwd(), `.inline-auto-removal-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'The real inline command still omits the daemon guide or creates pipeline state before rejecting --auto, proving the unattended one-shot removal boundary is not implemented.',
  };
} catch (error) {
  evidence = {
    executed: 0,
