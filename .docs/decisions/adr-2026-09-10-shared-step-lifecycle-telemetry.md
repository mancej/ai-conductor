...
> **Amended 2026-09-15 by #2414:** James Stoup approved the scope decision in the operator recovery session: preserving satisfied siblings as `done` on a no-verdict validation-group halt remains owned by #1425, under [validation-group join Decision 2](adr-2026-07-10-validation-group-join.md) and its [separate implementation plan](../plans/one-transient-failure-in-a-validation-group-member.md). This telemetry feature may ship without implementing that pre-existing retention gap; it must preserve existing join state/gate behavior and must not absorb #1425 or its #2190 dependency. As-built finding AB-2 is resolved as a feature-ownership decision, not as implemented retention or a satisfied retention check. AB-1 remains required: report aggregates and the engineer signal must preserve configured member identities under Decision 5 and Task 16. All other telemetry acceptance and validation requirements remain in force.
