...
| `run-fail` | `engineer run-fail --run-id <id> --error <text>` | Classifies and appends a terminal failure with bounded class, code, summary, retryability, remedy, and redacted diagnostic while preserving the raw compatible `error` field. A correlated retry uses a new attempt key and run id. | 0; 1 on an invalid or terminal run |
...
| `worktree-cleanup` | `engineer worktree-cleanup --run-id <id> --reason <operator_cleanup\|task_cancelled\|spec_merged\|spec_closed\|retention_expired>` | Validates the exact marker, repository, registered worktree, branch, and commit; records logical retirement first; then removes that one worktree. Identity-precondition and removal failures persist typed evidence in the inspectable `cleanup` projection with retry timing. Removal debt never creates a second retirement event. | 0 after recording the resulting snapshot; 1 on identity or transition failure |
| `maintenance` | `engineer maintenance` | Reconciles retained review worktrees across durable Engineer runs. PR merge, PR close, cancellation, and deadline expiry authorize retirement; open PRs remain available. | 0 after the sweep; 1 on invalid input or an unrecoverable store error |
...
| `handoff` | `engineer handoff --project <name> --branch <branch> --worktree <path> [--source-ref <ref>]` | Rechecks owned-run readiness, opens the spec PR with `gh` inside the per-idea worktree, durably records the exact retained commit and review deadline, and leaves the worktree registered. It prints `{kind:'pr-opened', url}` or `{kind:'local-commit', branch, repoPath, reason}` and starts the target daemon. Legacy unmarked worktrees remain supported and are retained without fabricated lifecycle evidence. With `--source-ref`, it performs the existing issue write-back. PR-open or readiness failure keeps the worktree. | 0 after delivered handoff, including warned advisory write-back failures; 1 on readiness, project, target, identity, or PR-open failure |
